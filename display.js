...
export function showTree(
  ...
  root = d3.hierarchy(theRoot, function (d) {
  ...
  while (q.length > 0) {
    ...
    if (onlyPaths) {
      if (pathsNodes.has(wtId) && !pathEndpoints.includes(wtId) && (expandPaths || n.data.isBelowGeneration(maxGen))) {
        if (n.children) {
      ...
      if (n.data.isBelowGeneration(maxGen) || (expandPaths && pathsNodes.has(wtId) && !pathEndpoints.includes(wtId))) {
        if (n.children) {
          for (const c of n.children) {
            q.push(c);
          }
  ...
  function update(source) {
    ...
    function toggleDuplicate(event, d) {
      ...
      let linksToUnmark;
      if (setMarked) {
        const [pathsRoot, nodes, links, wtIds] = Tree.findPaths([d.data.getWtId()]);
        linksToMark = links;
        markedNodes.add(id);
